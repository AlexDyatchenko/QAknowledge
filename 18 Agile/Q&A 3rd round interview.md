...
In a cloud-native, networking environment like Megaport's, resilience means the system's ability to "absorb" a shock and continue functioning, rather than crashing entirely.
resilient to failures - устойчивый к сбоям.

**2. How do you handle a situation where a developer pushes back on a bug you’ve reported?**

* **Answer:** I focus on the data. I provide clear reproduction steps, logs, and the potential impact on the customer. I approach it as a collaborator, not an adversary—asking, "How can we make this work for the user?" rather than "You made a mistake." adversary - противник. Example: Settnigs was changed from prod -> staging and causes yeserday's passed tests to fail.
...
Example: I did performance test for test task and will continue to do so.
...
Example: System is not working for a day. Outcome: need to get not yesterday state but last state or yesterday - currency rates approach.
Actions: mark tests with tags by subsystems, flows, to be able to run only needed (what was changed) part of test suite. Parallel execution in containers.
Multiple envs and influxDB instances to run performance metrics in parallel.
...
Ecample: Settings box that deleted all data then generated new set in DB. Parallel execution broke it. Table lock fixed the issue.
...
Action (The "Risk-Based" Argument): "I didn't just say 'No.' I prepared a Risk Assessment Matrix. I categorized the failing tests into:

    Likelihood: How often will a user hit this? (Low)

    Impact: If they hit it, what happens? (High—Data corruption).

    Workaround: Is there a way to fix it without a code change? (No).
